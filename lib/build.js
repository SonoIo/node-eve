...
module.exports = function(type, unit, watcher, options, done) {
	...
	var bowerJSONFile   = path.resolve('bower.json');
	var pkg             = require(packageJSONFile);
	var bower           = require(bowerJSONFile);
	...
	var outputCommonsJsFile = path.join('build', 'assets', 'js', 'commons.bundle.js');
	var now            = new Date();
	...
	// Write file
	fs.writeFileSync(commonJSFile, text, {encoding: 'utf8'});

	// Entry files
	b.require( [path.resolve(entryJsFile), commonJSFile], { entry: true });

	// Plugin Factor bundle. Split in tow bundles
	b.plugin('factor-bundle', { outputs: [ outputJsFile, commonBundleJSFile ] });
