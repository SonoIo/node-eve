...
const babelPluginBuiltinExtend             = require.resolve('babel-plugin-transform-builtin-extend');
const babelPluginTransformClassProperties  = require.resolve('babel-plugin-transform-class-properties');
const babelPluginCheckEs2015Constants      = require.resolve('babel-plugin-check-es2015-constants');
const babelPluginTransformEs2015Spread     = require.resolve('babel-plugin-transform-es2015-spread');
const babelPluginTransformEs2015RestSpread = require.resolve('babel-plugin-transform-object-rest-spread');
const babelPluginTransformExportExtensions = require.resolve('babel-plugin-transform-export-extensions');
const babelPluginInlineJsonImport          = require.resolve('babel-plugin-inline-json-import');
...
module.exports = function(type, unit, watcher, options, done) {
	...
	_.defaults(watchifyConfig, {
		delay: 500,
		ignoreWatch: [ '*.bundle.js', 'bundle.js', 'styles.css', 'package.json', 'config.json', '**/styles/**', '**/node_modules/**' ],
		poll: false
	});
	...
	if ( babelifyDir.length > 0  ){
		...
		let builtinExtendOptions = pkgBabelPlugins["builtin-extend"] || { "globals": [ "Error", "Worker" ], "approximate": false };

		let babelPlugins = [
			[babelPluginBuiltinExtend, builtinExtendOptions],
			babelPluginTransformClassProperties, // Questo cozza con babelPluginInlineJsonImport
			babelPluginCheckEs2015Constants,
			babelPluginTransformEs2015Spread,
			babelPluginTransformEs2015RestSpread,
	...
	// compile transalation
	function compileL10ns(done) {
		var componentsDir = path.resolve(bowerrc.directory);
		var directories   = fs.readdirSync( componentsDir, { encoding: 'utf8' }).filter(junk.not);
		var cwd           = process.cwd();
		var actions       = [];
