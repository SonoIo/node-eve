...
module.exports = function(type, unit, watcher, options, done) {
				...
				setTimeout(function() {
					...
					waterfall([
						...
						function (next) {
							...
							try{
								var code   = fs.readFileSync( outputCommonsJsFile, {encoding: 'utf8'} );
								var minify = UglifyJS.minify( code, UGLIFYJS_OPTIONS );
							}catch(e){
							...
							if ( minify && !minify.error ) {
								...
								if ( minify && minify.error )
									return next( new Error(minify.error.message) );
						...
						function (next) {
							...
							try {
								var code   = fs.readFileSync( outputJsFile, {encoding: 'utf8'} );
								var minify = UglifyJS.minify( code, UGLIFYJS_OPTIONS );
							} catch (e) {
								console.error("Error on parse file %s", e.filename);
								return next(e);
	...
	function searchES6IntoComponents(babelifyDir) {
		...
		// var bowerComponents = [];
		// try{
		// 	bowerComponents = fs.readdirSync( componentsDir, { encoding: 'utf8' }).filter(junk.not);
		// }catch(e){
		// 	bowerComponents = [];
		// }
		//
		// componentsDir = path.resolve('node_modules');
		// var npmComponents = [];
		// try{
		// 	npmComponents = fs.readdirSync( componentsDir, { encoding: 'utf8' }).filter(junk.not);
		// }catch(e){
		// 	npmComponents = [];
		// }

		// var directories = _.union(bowerComponents, npmComponents);
