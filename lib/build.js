	...
	function publisher(outputJsFile, mode, done){
		setTimeout(function(){
			...
			if ( accessKey && secretKey ){
				...
				console.log('  Crypt code with jScrambler...'.gray);
				var time   = process.hrtime();
					...
					.then(function (res) {
						...
						fs.createReadStream(zipFile)
							...
							.on('close', function(){
								...
								console.log('  Encryption completed in '.gray+beautifyTime(process.hrtime(time)).green);
							});
							
						return true;
					})
					.catch(function (error) {
						done(new Error(error && error.message ? 'jScrambler... ' + error.message : 'Error while encrypting code with jScrambler' ) );
					});
			}
