...
module.exports = function l10ns(cmd, options, done){
	...
	const l10nsJsonPath = path.resolve(process.cwd(), 'l10ns.json');

	try{
		const d = fs.openSync(l10nsJsonPath, 'r+');
		fs.closeSync(d);
	}catch(e){
		return done( new Error(`The l10ns.json is not found`) );
	}

	let l10nsJson = require(l10nsJsonPath);
	let keys = Object.keys(l10nsJson.projects);
	let l10nsConfig = l10nsJson.projects[keys[0]];
	let localizationsDir = path.resolve(process.cwd(), l10nsConfig.store);

	if ( cmd == "watch"){

		watch({ dirsAndFiles: ['lib', 'templates'] }, _.debounce( compileL10ns,  500 ) );
		watch({ dirsAndFiles: [l10nsConfig.store] }, _.debounce( inline,  500 ) );
		compileL10ns();
	...
	function inline() {
		...
		function processFile(lang) {
			let filepath = path.join(localizationsDir, lang + '.json');
